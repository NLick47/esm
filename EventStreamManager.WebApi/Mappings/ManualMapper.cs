...
public static class ManualMapper
{
    ...
    #region ProcessorPipeline (Request → Entity)

    public static ProcessorPipeline ToEntity(this CreatePipelineRequest request)
    {
        return new ProcessorPipeline
        {
            ...
            EventCodes = request.EventCodes,
            DatabaseTypes = request.DatabaseTypes,
            Stages = request.Stages.ToEntities(),
            Enabled = request.Enabled,
            MaxRetryCount = request.MaxRetryCount
        };
    }

    public static ProcessorPipeline ToEntity(this UpdatePipelineRequest request, string id)
    {
        return new ProcessorPipeline
        {
            Id = id,
    ...
    #region PipelineStage (Request → Entity)
    ...
    public static List<PipelineStage> ToEntities(this List<PipelineStageRequest>? requests)
    {
        return requests?.Select(r => r.ToEntity()).ToList() ?? new List<PipelineStage>();
    }
    ...
    #region ProcessorPipeline (Entity → Response)

    public static PipelineResponse ToResponse(this ProcessorPipeline pipeline)
    {
        return new PipelineResponse
        {
            Id = pipeline.Id,
            Name = pipeline.Name,
            EventCodes = pipeline.EventCodes,
            DatabaseTypes = pipeline.DatabaseTypes,
            Stages = pipeline.Stages.ToResponses(),
            Enabled = pipeline.Enabled,
            MaxRetryCount = pipeline.MaxRetryCount
        };
    }

    public static List<PipelineResponse> ToResponses(this List<ProcessorPipeline> pipelines)
    {
        return pipelines.Select(p => p.ToResponse()).ToList();
    }

    #endregion

    #region PipelineStage (Entity → Response)
    ...
    public static PipelineStageResponse ToResponse(this PipelineStage stage)
    {
        return new PipelineStageResponse
        {
            ...
            OnFailure = stage.OnFailure.ToString(),
            Condition = stage.Condition
        };
    }

    public static List<PipelineStageResponse> ToResponses(this List<PipelineStage>? stages)
    {
        return stages?.Select(s => s.ToResponse()).ToList() ?? new List<PipelineStageResponse>();
    }
