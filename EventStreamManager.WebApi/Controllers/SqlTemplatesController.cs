...
public class SqlTemplatesController : BaseController
{
    ...
    public async Task<IActionResult> GetSystemTemplates()
    {
        ...
        return Ok(list.ToResponses(), "获取系统模板列表成功");
    }
    ...
    public async Task<IActionResult> GetCustomTemplates()
    {
        ...
        return Ok(list.ToResponses(), "获取自定义模板列表成功");
    }
    
    [HttpPost("custom")]
    public async Task<IActionResult> CreateCustom([FromBody] CreateCustomSqlTemplateRequest request)
    {
        var template = request.ToEntity();
        var created = await _sqlTemplateService.CreateCustomAsync(template);
        return Ok(created.ToResponse(), "创建自定义模板成功");
    }
    
    [HttpPut("custom/{id}")]
    public async Task<IActionResult> UpdateCustom(string id, [FromBody] UpdateCustomSqlTemplateRequest request)
    {
        var template = request.ToEntity();
